...
class AdapterActivityList(private val activities: List<Any>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        if (getItemViewType(position) == 0) {
                ((holder as ActivityViewHolder)).bind(activities[position] as DataOfActivity)
        }
    ...
    override fun getItemViewType(position: Int): Int {
        return when {
            activities[position] is DataOfActivity -> 0
    ...
    inner class ActivityViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        ...
        fun bind(activity: DataOfActivity) {
            distance.text = activity.distance
            type.text = activity.activityType
            val duration_ = Duration.between(
                activity.endDate,
                activity.startDate
            );
            ...
            if (LocalDateTime.now().year == activity.endDate.year &&
                ...
                LocalDateTime.now().dayOfMonth == activity.endDate.dayOfMonth) { date.text = Duration.between(activity.endDate, LocalDateTime.now()).toHours().toString() +
    ...
    inner class DateViewHolder(itemView: View): RecyclerView.ViewHolder(itemView) {
        private val date = itemView.findViewById<TextView>(R.id.new_activity_fragment_label)
        @SuppressLint("SetTextI18n")
